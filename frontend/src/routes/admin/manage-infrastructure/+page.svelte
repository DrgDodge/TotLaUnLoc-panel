...
<div class="max-w-7xl mx-auto p-4 md:p-8">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    ...
    <div class="bg-neutral-800 p-8 rounded-xl shadow-2xl border border-neutral-700">
      <h3 class="text-3xl font-semibold mb-4 text-green-300">Connected Clients</h3>
      <div class="border border-neutral-700 rounded-lg p-6 bg-neutral-900 min-h-[200px]">
        {#if connectedSockets.length > 0}
          <ul>
            {#each connectedSockets as s}
              <li class="flex justify-between items-center p-2 rounded-lg hover:bg-neutral-800">
                <span class="font-mono text-sm text-neutral-300">{s.socketId}</span>
                <div class="flex gap-2">
        ...
        {:else}
          <p class="text-neutral-400 text-lg text-center pt-8">No clients connected.</p>
        {/if}
      </div>
    </div>
